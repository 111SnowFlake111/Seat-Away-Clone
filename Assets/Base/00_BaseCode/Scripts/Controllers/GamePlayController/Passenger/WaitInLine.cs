    ...
    public void MoveToNextTile()
    {
        ...
        if (waitTile.frontTile != null)
        {
            if (!waitTile.frontTile.occupied)
            {
                ...
                transform.DOLocalMove(waitTile.frontTile.transform.localPosition + new Vector3(0, 0.5f, 0), 0.25f).SetEase(Ease.Linear)
                    .OnStart(delegate
                {
        ...
        else if(waitTile.entrance != null)
        {
            if(!waitTile.entrance.occupied)
            {
                ...
                waitTile.occupied = false;
                transform.DOLocalMove(waitTile.entrance.transform.localPosition + new Vector3(0, 0.5f, 0), 0.25f).SetEase(Ease.Linear)
                    .OnStart(delegate
                {
                    GetComponent<Animator>().SetBool("Walk", true);
