...
public class GRemoteConfig
{
    ...
    public static bool GetJsonConfig<T>(string key, out bool result)
    {
        string input;
        result = true;
        if (!firebaseRemoteKeys.Contains(key) /*|| string.IsNullOrEmpty(input = FirebaseRemoteConfig.DefaultInstance.GetValue(key).StringValue)*/)
        {
            result = default;
            return false;
